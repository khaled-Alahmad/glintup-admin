...
// ق��ئمة الخدمات المتاحة مع الأسعار والمدد
...
export default function AddAppointment() {
  ...
  const handleAddService = () => {
    ...
    const serviceExists = selectedServices.find((s) => s.id === currentServiceId)

    if (serviceExists) {
      // زيادة الكمية إذا كانت الخدمة موجودة بالفعل
      setSelectedServices((prev) =>
        prev.map((service) =>
          service.id === currentServiceId ? { ...service, quantity: service.quantity + 1 } : service,
        ),
      )
    } else {
      // إضافة خدمة جديدة
      const serviceToAdd = availableServices.find((s) => s.id === currentServiceId)
      if (serviceToAdd) {
        setSelectedServices((prev) => [...prev, { ...serviceToAdd, quantity: 1 }])
      }
    }
  ...
  const handleUpdateQuantity = (id: string, quantity: number) => {
    ...
    setSelectedServices((prev) => prev.map((service) => (service.id === id ? { ...service, quantity } : service)))
  }
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex gap-3 items-end">
              ...
              <Button type="button" onClick={handleAddService} disabled={!currentServiceId} className="mb-0.5">
            ...
            {selectedServices.length > 0 ? (
                  ...
                  <tbody>
                    {selectedServices.map((service) => (
                      <tr key={service.id} className="border-t">
                        ...
                        <td className="p-3 text-center font-medium">{service.price * service.quantity}</td>
                        <td className="p-3 text-center">
                          <Button
                            type="button"
                            variant="ghost"
              ...
              <div className="text-center py-6 border rounded-md bg-muted/20">
                <p className="text-muted-foreground">لم يتم إضافة أي خدمات بعد</p>
              </div>
            )}
